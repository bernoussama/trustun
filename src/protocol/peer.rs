...
use super::wire::{HandshakeInit, HandshakeResp, TransportData, WirePacket};
use bincode::Options;
use rand::Rng;
...
const MAX_PAYLOAD: usize = crate::TUN_MTU; // Match TUN MTU
        ...
        match input {
            Input::UdpPacket(data, addr) => {
                let packet: WirePacket = bincode::options()
                    .with_limit(crate::WIRE_MTU as u64)
                    .with_little_endian()
                            ...
                            if !noise.is_initiator() {
                                ...
                                match noise.read_message(&message, &mut buf) {
                                    Ok(_) => {
                                        ...
                                        let resp_packet =
                                            WirePacket::HandshakeResp(HandshakeResp {
                                                sender_index: self.local_index,
